...
# extract banking card
SERVICE_ACCOUNT_JSON = os.environ.get("BANKING_CARD_DOCUMENTAI_CREDENTIALS")
if not SERVICE_ACCOUNT_JSON:
    raise RuntimeError(
        "Set BANKING_CARD_DOCUMENTAI_CREDENTIALS to the service account JSON path "
        "(copy from .env.example; file must not be committed)."
    )
PROJECT_ID = "projectbao-virpluz"
LOCATION = "us"
PROCESSOR_ID = "79797e45e0b3fe66"  # banking card processor id
...
def process_banking_card_ocr(image_bytes: bytes) -> dict:
...
async def extract_banking_card_info_from_media(url: str) -> dict:
    try:
        image_bytes = await download_image_to_memory(url)
        if image_bytes is None:
            raise ValueError("Failed to download image")

        result = process_banking_card_ocr(image_bytes)

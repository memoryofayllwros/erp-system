...
# extract national id
SERVICE_ACCOUNT_JSON = os.environ.get("NATIONAL_ID_DOCUMENTAI_CREDENTIALS")
if not SERVICE_ACCOUNT_JSON:
    raise RuntimeError(
        "Set NATIONAL_ID_DOCUMENTAI_CREDENTIALS to the service account JSON path "
        "(copy from .env.example; file must not be committed)."
    )
PROJECT_ID = "projectbao-virpluz"
LOCATION = "us"
PROCESSOR_ID = "e9cecb7c3c747fd7"
# extract national id
...
def process_national_id_ocr(image_bytes: bytes) -> dict:
...
async def extract_national_id_info_from_media(url: str) -> dict:
    try:
        image_bytes = await download_image_to_memory(url)
        if image_bytes is None:
            raise ValueError("Failed to download image")

        result = process_national_id_ocr(image_bytes)

        return result

    except Exception as e:
        logging.error(f"Error in national id OCR processing from URL: {str(e)}")
        raise ValueError(f"Error in national id OCR processing: {str(e)}")

...
SERVICE_ACCOUNT_JSON = os.environ.get("MATERIAL_OCR_DOCUMENTAI_CREDENTIALS")
if not SERVICE_ACCOUNT_JSON:
    raise RuntimeError(
        "Set MATERIAL_OCR_DOCUMENTAI_CREDENTIALS to the service account JSON path "
        "(copy from .env.example; file must not be committed)."
    )
PROJECT_ID = "projectbao-virpluz"
LOCATION = "us"
PROCESSOR_ID = "de1abe0931005403"
...
def process_material_ocr(pdf_bytes: bytes) -> dict:
...
async def download_pdf_to_memory(media_url: str) -> bytes:
    if not media_url:
        return None
...
async def extract_material_info_from_media(url: str) -> dict:
    try:
        pdf_bytes = await download_pdf_to_memory(url)

        result = process_material_ocr(pdf_bytes)

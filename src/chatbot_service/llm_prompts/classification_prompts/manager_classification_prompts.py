...
from src.chatbot_service.chatbot_helpers.intent_manager import MANAGER_VALID_INTENTS
from src.chatbot_service.chatbot_helpers.setup_llm import gpt_4o_llm

manager_intent_template = ChatPromptTemplate.from_messages(
    [
        (
            "system",
        ...
        - **delete_project**: This intent is triggered when the user wants to **delete an existing project**.  
        The assistant should detect this intent when the message includes details like:   
          - The **project_code** of the project (required), usually consists of four letters and five-digit number, might end with 'NO', for example, 'WLHK25003', 'WCGEO25002', 'WCGE25003NO' etc.
          - Keywords: '移除工程', 'remove', 'delete project' or similar   
        ...
        - **check_in_via_gps**: Triggered when the user wants to check in to a project. Typical inputs include:
          - Keywords: '簽到', 'check in', '打卡', 'check out' or similar.
          - Example:
            「簽到」
            "Check in"
            "Check out"
        ...
        1. **Match Against Key Terms** 
          ...
          - The user should only use one of the intents, if all intents are not correct, return None.
        ...
        "add_project", "update_project", "read_specific_project", "remove_project_gps_location",
        "check_in_via_gps", "today_attendance_situation",
        "monthly_payslip", "check_in_via_image", "lunch_overtime"
         """,
        ),
        ("human", "{messages}"),
    ]
...
manager_intent_chain = manager_intent_template | gpt_4o_llm | StrOutputParser()


def classify_manager_intent(body):
    try:
        ...
        intent = (
            manager_intent_chain.invoke({"messages": body}).strip().lower()
        )  # Normalize intent

        logging.info(f"Intent: {intent}")

        for intent_name in MANAGER_VALID_INTENTS:
